...
class FileRepository:
    ...
    def add_file(self, file_hash, filename, filetype, filesize):
        app.logger.info("Adding new file. File-md5: '{}'; Filename: '{}'".format(file_hash, filename))
        server_current_time = time.strftime("%d/%m/%Y - %H:%M:%S")
        self.files.upsert({'file_hash': file_hash, 'filename': filename, 'filetype': filetype.lower(), 'result': 'N/A', 'datetime': server_current_time, 'analysis_time': 'N/A', 'filesize': filesize}, self.query.file_hash == file_hash)
    ...
    def get_files_by_filename(self, filename):
        ...
        pattern = ".*" + filename + ".*"
        a = self.files.search(self.query.filename.matches(pattern, flags=re.IGNORECASE))
        return sorted(a, key=lambda r: time.strptime(r['datetime'], "%d/%m/%Y - %H:%M:%S"), reverse=True)

    def get_files_by_filehash(self, file_hash):
        app.logger.info("Getting files by md5: '{}'".format(file_hash))
        pattern = ".*" + file_hash + ".*"
        a = self.files.search(self.query.file_hash.matches(pattern, flags=re.IGNORECASE))
        return sorted(a, key=lambda r: time.strptime(r['datetime'], "%d/%m/%Y - %H:%M:%S"), reverse=True)

    def get_files_by_datetime(self, datetime):
        app.logger.info("Getting files by datetime: '{}'".format(datetime))
        pattern = ".*" + datetime + ".*"
        a = self.files.search(self.query.datetime.matches(pattern, flags=re.IGNORECASE))
        return sorted(a, key=lambda r: time.strptime(r['datetime'], "%d/%m/%Y - %H:%M:%S"), reverse=True)

    def get_files_by_filetype(self, filetype):
        app.logger.info("Getting files by filetype: '{}'".format(filetype))
        pattern = ".*" + filetype + ".*"
        a = self.files.search(self.query.filetype.matches(pattern, flags=re.IGNORECASE))
        return sorted(a, key=lambda r: time.strptime(r['datetime'], "%d/%m/%Y - %H:%M:%S"), reverse=True)

    def get_files_by_result(self, result):
        app.logger.info("Getting files by type: '{}'".format(result))
        pattern = ".*" + result + ".*"
        a = self.files.search(self.query.result.matches(pattern, flags=re.IGNORECASE))
        return sorted(a, key=lambda r: time.strptime(r['datetime'], "%d/%m/%Y - %H:%M:%S"), reverse=True)
